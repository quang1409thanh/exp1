						  ...
						  xpu_datum_t *__result)
{
	...
	case ArrowDateUnit__Day:
		...
		{
			result->expr_ops = &xpu_date_ops;
			result->value = *((uint32_t *)addr) - (POSTGRES_EPOCH_JDATE - UNIX_EPOCH_JDATE);
		}
		break;
	...
	case ArrowDateUnit__MilliSecond:
		...
		{
			result->expr_ops = &xpu_date_ops;
			result->value = *((uint64_t *)addr) / (SECS_PER_DAY * 1000) - (POSTGRES_EPOCH_JDATE - UNIX_EPOCH_JDATE);
		}
		break;
...
pgfn_timestamp_time(XPU_PGFUNCTION_ARGS)
{
	...
	else
	{
		...
		{
			result->expr_ops = &xpu_time_ops;
			result->value = ((((tm.tm_hour * MINS_PER_HOUR + tm.tm_min) * SECS_PER_MINUTE) + tm.tm_sec) * USECS_PER_SEC) + fsec;
		}
		else
		{
			STROM_ELOG(kcxt, "timestamp out of range");
...
pgfn_extract_date(XPU_PGFUNCTION_ARGS)
{
	...
	else if (type == RESERV && value == DTK_EPOCH)
	{
		...
		result->expr_ops = &xpu_numeric_ops;
		result->kind = XPU_NUMERIC_KIND__VALID;
		result->u.value = ((int64_t)dval.value + POSTGRES_EPOCH_JDATE - UNIX_EPOCH_JDATE) * SECS_PER_DAY;
	}
	else
	{
	...
	else if (type == RESERV && value == DTK_EPOCH)
	{
		...
		result->u.value =
			(((int64_t)ival.value.month / MONTHS_PER_YEAR) * SECS_PER_YEAR +
			 ...
			 (int64_t)ival.value.day * SECS_PER_DAY) *
				1000000L +
