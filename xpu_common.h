...
#define PageGetMaxOffsetNumber(page) \
	(((PageHeaderData *)(page))->pd_lower <= SizeOfPageHeaderData ? 0 : ((((PageHeaderData *)(page))->pd_lower - SizeOfPageHeaderData) / sizeof(ItemIdData)))
...
KDS_HASH_FIRST_ITEM(const kern_data_store *kds, uint32_t hash)
{
	...
	{
		kern_hashitem *hitem = (kern_hashitem *)((char *)kds + kds->length - offset);
...
KDS_HASH_NEXT_ITEM(const kern_data_store *kds, uint64_t hnext_offset)
{
	...
	{
		kern_hashitem *hnext = (kern_hashitem *)((char *)kds + kds->length - hnext_offset);
							  ...
							  int *p_length)
{
	...
	{
		const uint32_t *offset = (const uint32_t *)((const char *)kds + cmeta->values_offset);
		const char *extra = (const char *)((const char *)kds + cmeta->extra_offset);
							  ...
							  int *p_length)
{
	...
	if (sizeof(uint32_t) * (index + 1) <= cmeta->values_length)
	{
		const uint64_t *offset = (const uint64_t *)((const char *)kds + cmeta->values_offset);
		const char *extra = (const char *)((const char *)kds + cmeta->extra_offset);
		if (offset[index] <= offset[index + 1] &&
			offset[index + 1] <= cmeta->extra_length &&
			offset[index + 1] - offset[index] <= VARATT_MAX)
...
#define VARTAG_SIZE(tag)                                                                                   \
	((tag) == VARTAG_INDIRECT ? sizeof(varatt_indirect) : (tag) == VARTAG_ONDISK ? sizeof(varatt_external) \
																				 : 0 /* should not happen */)
				...
				const kern_expression *karg)
{
	if (kexp)
	{
			...
			(char *)karg + karg->len <= (char *)kexp + kexp->len)
		{
			...
			if (*magic == (KERN_EXPRESSION_MAGIC ^ ((uint32_t)karg->exptype << 6) ^ ((uint32_t)karg->opcode << 14)))
			{
				/* ok, it looks valid */
				return true;
			}
...
SESSION_KEXP_LOAD_VARS(const kern_session_info *session, int depth)
{
	...
	if (session->xpucode_load_vars_packed == 0)
		return NULL;
	kexp = (kern_expression *)((char *)session + session->xpucode_load_vars_packed);
	if (depth < 0)
		return kexp;
...
SESSION_KEXP_MOVE_VARS(const kern_session_info *session, int depth)
{
	...
	if (session->xpucode_move_vars_packed == 0)
		return NULL;
	kexp = (kern_expression *)((char *)session + session->xpucode_move_vars_packed);
	if (depth < 0)
		return kexp;
	karg = __PICKUP_PACKED_KEXP(kexp, depth);
...
SESSION_KEXP_GIST_EVALS(const kern_session_info *session, int depth)
{
	...
	if (session->xpucode_gist_evals_packed == 0)
		return NULL;
	kexp = (kern_expression *)((char *)session + session->xpucode_gist_evals_packed);
	if (depth < 0)
		return kexp;
	karg = __PICKUP_PACKED_KEXP(kexp, depth);
...
SESSION_KEXP_GROUPBY_KEYHASH(const kern_session_info *session)
{
	...
	{
		kexp = (kern_expression *)((char *)session + session->xpucode_groupby_keyhash);
...
SESSION_KEXP_GROUPBY_KEYLOAD(const kern_session_info *session)
{
	...
	{
		kexp = (kern_expression *)((char *)session + session->xpucode_groupby_keyload);
...
SESSION_KEXP_GROUPBY_KEYCOMP(const kern_session_info *session)
{
	...
	{
		kexp = (kern_expression *)((char *)session + session->xpucode_groupby_keycomp);
...
SESSION_KEXP_GROUPBY_ACTIONS(const kern_session_info *session)
{
	...
	{
		kexp = (kern_expression *)((char *)session + session->xpucode_groupby_actions);
...
#define FUNC_OPCODE(a, b, c, NAME, d, e) \
	EXTERN_FUNCTION(bool)                \
	pgfn_##NAME(XPU_PGFUNCTION_ARGS);
...
KERN_MULTIRELS_PARTITION_DESC(kern_multirels *kmrels, int depth)
{
	...
	if (offset > 0)
	{
		kern_buffer_partitions *kbuf_parts = (kern_buffer_partitions *)((char *)kmrels + offset);
