...
pgfn_CaseWhenExpr(XPU_PGFUNCTION_ARGS)
{
	...
	{
		karg = (const kern_expression *)((char *)kexp + kexp->u.casewhen.case_comp);
	...
	{
		karg = (const kern_expression *)((char *)kexp + kexp->u.casewhen.case_else);
		assert(__KEXP_IS_VALID(kexp, karg) && kexp->exptype == karg->exptype);
		if (!EXEC_KERN_EXPRESSION(kcxt, karg, __result))
			return false;
	}
					 ...
					 xpu_array_t *aval)
{
	...
	result->value = !use_any;
	kds = (const kern_data_store *)((char *)cmeta - cmeta->kds_offset);
								 ...
								 const kern_varload_desc *vl_desc)
{
	...
	sysatt = (GpuCacheSysattr *)((char *)kds + cmeta->values_offset);
	switch (vl_desc->vl_resno)
	{
	case SelfItemPointerAttributeNumber:
						...
						int dst_kvec_id)
{
	...
	for (int i = 0; i < kexp_move_vars->u.move.nitems; i++)
	{
		...
		assert(vm_desc->vm_offset +
				   vs_desc->vs_ops->xpu_kvec_sizeof <=
			   kcxt->kvecs_bufsz);
						   ...
						   bool matched)
{
	...
	fbitem = (kern_fallbackitem *)((char *)kds_fallback + kds_fallback->length - __usage);
	fbitem->t_len = tupsz;
	fbitem->depth = depth;
	...
	__kern_form_heaptuple(kcxt,
						  ...
						  kcxt->kds_fallback,
						  &fbitem->htup);
	KDS_GET_ROWINDEX(kds_fallback)
	[__nitems_old] = __usage;
						 ...
						 bool matched)
{
		...
		kcxt->kds_fallback != NULL)
	{
		...
		kern_fallback_desc *fb_desc_array = (kern_fallback_desc *)((char *)session + session->fallback_desc_defs);
		...
		for (int j = 0; j < fallback_nitems; j++)
		{
			...
			{
				const kern_varslot_desc *vs_desc = &kcxt->kvars_desc[slot_id];
				const kvec_datum_t *kvecs = (const kvec_datum_t *)((char *)kcxt->kvecs_curr_buffer + vs_desc->vs_offset);
						   ...
						   xpu_datum_t *__result)
{
	...
	{
		const uint32_t *base = (const uint32_t *)((const char *)kds + cmeta->values_offset);
	...
	{
		const uint64_t *base = (const uint64_t *)((const char *)kds + cmeta->values_offset);
						...
						const xpu_array_t *arg)
{
	...
	kds = (const kern_data_store *)((const char *)cmeta - cmeta->kds_offset);
	assert(cmeta->num_subattrs == 1);
							...
							const xpu_composite_t *arg)
{
	...
	kds = (const kern_data_store *)((const char *)cmeta_dst - cmeta_dst->kds_offset);
	smeta_dst = &kds->colmeta[cmeta_dst->idx_subattrs];
	/* setup source cmeta/vs_desc */
	kds = (const kern_data_store *)((const char *)cmeta_src - cmeta_src->kds_offset);
